...
start_date, end_date = st.sidebar.date_input("Select timeframe", value=(default_start, today),
...
if uploaded_file:
    ...
    # --- Metrics Summary ---
    st.caption(
        f"Data from **{start_date.strftime('%d %b %Y')}** to **{end_date.strftime('%d %b %Y')}**")
    ...
    col4.metric("Avg. Breakdown Duration", format_hours_to_h_m(
        df['breakdown dur.'].mean()) if 'breakdown dur.' in df.columns else "N/A")
    col5.metric("Avg. Repair Time (MTTR)", format_hours_to_h_m(mttr_value))
    col6.metric("Total Downtime", format_hours_to_h_m(
        df['breakdown dur.'].sum()) if 'breakdown dur.' in df.columns else "N/A")
    st.markdown("---")
    ...
    with tab1:
        ...
        with col1:
            ...
            for machine in all_equipment:
                ...
                if not row.empty:
                    ...
                    machine_status.append(
                        {"equipment": machine, "status": "Down" if pd.Timestamp(end) > now else "Working"})
                else:
                    machine_status.append(
            ...
            fig = px.pie(pie_data, names="Status", values="Count", hole=0.5,
                         color="Status", color_discrete_map={"Working": "#41d5e8", "Down": "#c90115"})
        ...
        with col3:
            fig = px.bar(mtbf_df.sort_values(by='mtbf (hrs)').head(10),
                         x='equipment', y='mtbf (hrs)', title="Lowest MTBF",
        ...
        with col4:
            ...
            fig = px.bar(mttr_df.sort_values(by='repair time (hrs)', ascending=False).head(10),
                         x='equipment', y='repair time (hrs)', title="Highest MTTR",
                         color='repair time (hrs)', color_continuous_scale='blues')
            st.plotly_chart(fig, use_container_width=True)
    ...
    with tab2:
        ...
        for machine_id in filtered_df["equipment"].unique():
            ...
            if pd.notna(mtbf_val) and pd.notna(max_mtbf) and max_mtbf > 0:
                ...
                eta_text = format_hours_to_h_m(mtbf_val)
            else:
                tag = "No Score"
                health_percent = "N/A"
                eta_text = "N/A"
